    ...
    default:
      return (
        <button
          ...
          disabled={disabled}
          onClick={onClick}
          className={`${className} inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium rounded-lg group bg-gradient-to-br from-pink-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-white text-white focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800" ${
            disabled
              ? "cursor-not-allowed pointer-events-none opacity-50" // Adjust styling for disabled state
              : ""
          }`}
        >
          <span
            className={`${className} px-3 py-2.5 transition-all ease-in duration-75 text-whiterounded-md group-hover:bg-opacity-0`}
          >
            {label}
          </span>
        </button>
